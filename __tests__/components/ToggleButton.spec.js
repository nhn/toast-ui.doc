...
describe('ToggleButton component', () => {
  ...
  test('is closed.', () => {
    const wrapper = shallow(<ToggleButton opened={false} />);

    expect(wrapper.find('.btn-toggle').hasClass('opened')).toBe(false);
  });

  test('is opened.', () => {
    const wrapper = shallow(<ToggleButton opened={true} />);
  ...
  test('simulates click events.', () => {
    ...
    const wrapper = shallow(<ToggleButton handleClick={onButtonClick} />);
